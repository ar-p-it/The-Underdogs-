...
export default function MyGroups() {
  ...
  const visibleGroups = useMemo(() => {
    ...
    return (groups || []).filter((g) => {
      ...
      const isParticipant = Array.isArray(g.participants) && g.participants.some((p) => {
    ...
    <div className="min-h-screen bg-white text-black flex">
      ...
      <main className="flex-1 container mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {userEmail && (
            <div className="text-sm text-gray-600">Signed in as <span className="font-medium">{userEmail}</span></div>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {visibleGroups.map((g) => (
            <motion.div key={g._id} className="card border-t bg-white shadow-md hover:shadow-lg transition-shadow" whileHover={{ y: -4, scale: 1.01 }} transition={{ type: 'spring', stiffness: 120, damping: 12 }}>
              <div className="card-body ">
                ...
                <div className="mt-2">
                  ...
                  <div className="mt-1 text-xs text-gray-500">Admin: {String(g.admin)}</div>
                  <div className="mt-3">
                      <a href={`/groups/${g._id}`} className="btn btn-primary bg-emerald-500 border-none">Open Group</a>
                      <button
                        type="button"
                        className="btn ml-2"
                        onClick={() => { setInviteGroup(g); setShowInvite(true); }}
        ...
        <InviteQRModal
          ...
          onClose={() => { setShowInvite(false); setInviteGroup(null); }}
        />
      </main>
    </div>
  );
