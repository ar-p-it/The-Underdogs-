...
export default function GroupLedger() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [gRes, eRes, bRes] = await Promise.all([
        ...
        axios.get(`${API_BASE}/groups/${groupId}/expenses`, {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        splits: inputSplits
          ...
          .map((s) => ({
            ...
            percent:
              splitMethod === "percent" ? Number(s.percent || 0) : undefined,
            shares:
              splitMethod === "shares" ? Number(s.shares || 0) : undefined,
          })),
      };
      ...
      // Refresh balances
      const bRes = await axios.get(`${API_BASE}/groups/${groupId}/balances`, {
        withCredentials: true,
      });
      setBalances(bRes.data?.balances || []);
      setSettlements(bRes.data?.settlements || []);
  ...
  const renderInputs = () => {
    if (splitMethod === "equal") {
      return (
        ...
        >
          <FaCheckCircle /> Split equally among all {participants.length}{" "}
          participants.
        </motion.div>
      );
    }
        ...
        {participants.map((p, idx) => {
          const handleSplitChange = (e) => {
            ...
            if (splitMethod === "exact") clone[idx].amount = e.target.value;
              ...
              clone[idx].percent = e.target.value;
            ...
            >
              <label className="label">
                <span className="label-text font-medium text-slate-600">
                  {p.user.firstName || "Member"} {p.user.lastName || ""}
                </span>
              </label>
              <input
                type="number"
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-12">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {error && (
          <div className="alert alert-error mb-6 shadow-lg">{error}</div>
        )}
        ...
        {group && (
          <>
            ...
            >
              ...
              >
                <FaReceipt className="inline mr-2" /> Expenses
              </button>
              <button
              ...
              >
                <FaMoneyBillWave className="inline mr-2" /> Payment Pool
              </button>
            </motion.div>
                      ...
                      <form onSubmit={onSubmit} className="space-y-4">
                        <div className="grid md:grid-cols-3 gap-4">
                          ...
                          <div className="form-control">
                            ...
                            <select
                              className="select select-bordered w-full bg-white border-slate-300"
                              value={currency}
                              ...
                              <div className="avatar placeholder">
                                <div className="bg-emerald-200 text-emerald-900 rounded-full w-8">
                                  <span className="text-xs font-bold">
                                    {(b.user.firstName || "M")[0]}
                                  </span>
                                </div>
                              </div>
                              <span className="font-medium">
                                {b.user.firstName || "Member"}
                              </span>
                            </div>
                            <div
                              className={`font-mono font-bold text-lg ${b.balance >= 0 ? "text-emerald-200" : "text-red-200"}`}
                            >
                              {b.balance >= 0 ? "+" : ""}
                              <span ref={(el) => (balanceRefs.current[i] = el)}>
                                0
                              </span>
                            </div>
                          </div>
                  ...
                  >
                    <div className="card-body">
                      <h2 className="card-title text-slate-700 flex items-center gap-2">
                        <FaExchangeAlt className="text-emerald-500" /> Suggested
                        Settlements
                      </h2>
                      <p className="text-xs text-slate-400 mb-4">
                        The most efficient way to clear debts.
                      </p>
                      <div className="space-y-3">
                        {settlements.map((s, i) => {
                          ...
                          const fromName =
                            `${fromUser?.firstName || "Member"} ${fromUser?.lastName || ""}`.trim();
                          const toName =
                            `${toUser?.firstName || "Member"} ${toUser?.lastName || ""}`.trim();
                          return (
                            <motion.div
                    ...
                    <div className="card-body py-4">
                      ...
                      <div className="flex -space-x-2 overflow-hidden py-1">
                        {participants.map((p, i) => (
                          <div
                            key={i}
                            className="inline-block h-8 w-8 rounded-full ring-2 ring-white bg-emerald-100 flex items-center justify-center text-xs font-bold text-emerald-800"
                            title={p.user.emailId}
                          >
