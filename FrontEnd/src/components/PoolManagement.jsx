    ...
    <div className="space-y-6">
      ...
      {pool && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="card bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-white shadow-xl"
        >
          <div className="card-body">
            <h2 className="card-title text-2xl mb-6">💰 Pool Management</h2>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {/* Total Pool Amount */}
              <div className="flex flex-col bg-black bg-opacity-20 p-4 rounded-lg">
                <span className="text-sm opacity-90 font-semibold">
                  Total Pool
                </span>
                <span className="text-3xl font-bold flex items-center gap-2 mt-2">
                  <FaCoins className="text-yellow-300" />
                  {pool.totalAmount}
                </span>
                <span className="text-xs opacity-75">{pool.currency}</span>
              </div>
              ...
              <div className="flex flex-col bg-black bg-opacity-20 p-4 rounded-lg">
                ...
                <span className="text-3xl font-bold text-green-300 mt-2">
              ...
              {/* Locked Amount */}
              <div className="flex flex-col bg-black bg-opacity-20 p-4 rounded-lg">
                <span className="text-sm opacity-90 font-semibold">Locked</span>
                <span className="text-3xl font-bold text-red-300 mt-2">
      ...
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="card bg-white shadow-lg border-2 border-slate-100 hover:border-purple-300 transition-all"
      >
        <div className="card-body">
          ...
          <form onSubmit={handleAddMilestone} className="space-y-4">
            ...
            <div className="flex justify-end pt-2">
              <button
                type="submit"
                disabled={submitting}
                className="btn bg-purple-500 hover:bg-purple-600 text-white border-none gap-2"
              >
      ...
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="card bg-white shadow-lg border-2 border-slate-100 hover:border-green-300 transition-all"
      >
        <div className="card-body">
      ...
      {pool?.contributions && pool.contributions.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="card bg-white shadow-lg border-2 border-slate-100 hover:border-blue-300 transition-all"
        >
          <div className="card-body">
            <h2 className="card-title text-slate-800 mb-5 flex items-center gap-3">
            ...
            <div className="space-y-3">
              {pool.contributions.map((c) => (
                <motion.div
                  ...
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="flex justify-between items-center p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 hover:border-blue-400 transition-all"
                >
